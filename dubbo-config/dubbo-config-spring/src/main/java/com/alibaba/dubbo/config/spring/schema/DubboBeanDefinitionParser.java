...
public class DubboBeanDefinitionParser implements BeanDefinitionParser {
     ...
     * 解析XML的主流程
     *
     * @see #parseProperties    解析serviceBean的属性并赋值
     * @see #parseNested    解析内嵌的指向的子 XML 元素
     * @see #parseMultiRef  解析多指向的情况
     * @see #parseMethods   解析 <dubbo:method /> 标签
     * @see #parseArguments    解析 <dubbo:argument /> 标签
     * @see #parseParameters    解析 <dubbo:parameter /> 标签
     */
    ...
    private static BeanDefinition parse(Element element, ParserContext parserContext, Class<?> beanClass, boolean required) {
        ...
        if ((id == null || id.length() == 0) && required) {
            // 生成 id 。不同的配置对象，会存在不同。 规则为 name > 特殊规则 > className 。
        ...
        if (ProtocolConfig.class.equals(beanClass)) {
            ...
            // http://svip.iocoder.cn/Dubbo/configuration-xml/#
    ...
    private static void parseMethods(String id, NodeList nodeList, RootBeanDefinition beanDefinition, ParserContext parserContext) {
        if (nodeList != null && nodeList.getLength() > 0) {
            ManagedList methods = null; // 解析的方法数组methods
            for (int i = 0; i < nodeList.getLength(); i++) {
                Node node = nodeList.item(i);
                if (node instanceof Element) {
                    Element element = (Element) node;
                    // 这三行，判断只解析 `<dubbo:method />`
